...
Changelog
---------
Version 4.9.0.9: Major internal overhaul to fully support multiclass races - opponents and positions are now tracked correctly for the player's car class, the app doesn't make calls about opponent laptimes, pitting, and other stuff if they're not in the same class as the player; added multi-class specific messages; added multi-class specific voice commands "is the car ahead in my class", "is the car behind my class", "what class is the car ahead", :"what class is the car behind" and variations; work-in-progress RF2 updates; speculative fixes for driver swaps in iRacing; more performance improvements; fixed 'enable driver names' flag being ignored for some messages; added optional pit box countdown (R3E and iRacing only, enable with property "pit_box_position_countdown"); rF2 improvements: improved rolling start detection and distance to safety car detection;  in SCR mode, CC now respects DoubleFileType value, and would announce last FCY lap Frozen Order instructions correctly, added option to change left/right side tyres(iRacing only).
