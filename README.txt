...
Changelog
---------
Version 4.9.3.7: Added option to pause most messages in braking and cornering zones (sections of the track starting from heavy braking and ending with near-full throttle application) - this doesn't apply to spotter messages or other very high priority messages. It can be enabled and disabled with a button mapping ('Toggle delay messages on hard parts') or a voice command ('don't talk in the [corners/braking zones]' / 'no messages in the [corners/braking zones]', 'talk to me anywhere' / 'messages at any point'). The initial state is set with the 'Delay messages on hard parts' option and defaults to 'off'. The initial state is set with the 'Delay messages on hard parts' option. Note that this feature is disabled for ovals and may produce odd results on very short tracks; added some basic message prioritisation - there's still more work to do here; prevent any further fragments of a message from playing if this message was interrupted; simplified pitstop benchmark process - you can now get a benchmark pitstop time without having to set a hot lap first (although you'll still need to set one after your stop to allow the app to work out the time loss); a few bug fixes
