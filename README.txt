...
Changelog
---------
Version 4.9.5.8: Some fuel calculation tweaks - near the start of the race, use the max per-lap consumption when calculating required fuel; Save pitstop benchmark times for each car / track / game combination (written to /Documents/CrewChiefV4/pit_benchmarks.json) - enable with the 'Save pitstop benchmark times to disk' property (note this is *disabled* by default); RF2 - workaround for missing spotter calls after driver swaps (thanks to kcr55 for this one); RF2 - use tyre compound name rather than index when deriving tyre types - should fix issues where mods declare different available tyre compound sets; pCars2 - make macro key presses a bit longer
