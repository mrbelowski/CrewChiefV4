...
Changelog
---------
Version 4.9.0.8: Major internal overhaul to fully support multiclass races - opponents and positions are now tracked correctly for the player's car class, the app doesn't make calls about opponent laptimes, pitting, and other stuff if they're not in the same class as the player; speculative fixes for driver swaps in iRacing; more performance improvements; fixed 'enable driver names' flag being ignored for some messages; added optional pit box countdown (R3E only, enable with property "pit_box_position_countdown")
